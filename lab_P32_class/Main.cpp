...
bool read_file(string file_name, MOSFET** data, int* length) {
	...
	string new_name, new_type;// создаем переменные для свойств
	for (size_t i = 0; i < *length; i++) { //присваеваем новому масиву значения из фаила
		...
		local_data[i].set_quantity(new_quantity);//присваеваем новые значения свойствам объекта
	}
...
void EDIT(MOSFET** data, int line, int length) {
	...
	new_data[line].edit_console(new_data, length);//вызываем метод для изменения свойств
}
